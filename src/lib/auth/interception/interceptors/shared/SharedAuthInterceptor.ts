...
export default class SharedAuthInterceptor extends Interceptor {
  ...
  add() {
    ...
    this.dal.interceptors.request.use(async (request) => {
      const accessToken = await this.baseInterceptor.cookieManager.get(
        ACCESS_TOKEN_COOKIE_NAME,
      );
      if (accessToken) {
        request.headers["Authorization"] =
          `Bearer ${await this.baseInterceptor.cookieManager.get(ACCESS_TOKEN_COOKIE_NAME)}`;
      } else {
  ...
  protected async refreshAuthLogic(failedRequest: {response: AxiosResponse}) {
    return new Promise(async (resolve, reject) => {
      try {
        const refreshToken = await this.baseInterceptor.cookieManager.get(
          REFRESH_TOKEN_COOKIE_NAME,
        );
        if (!refreshToken) {
          await this.baseInterceptor.cookieManager.remove(
            ACCESS_TOKEN_COOKIE_NAME,
          );
        ...
        axios
          ...
          .then((response) => {
            ...
            failedRequest.response.config.headers["Authorization"] =
              "Bearer " + access_token;
            this.baseInterceptor.cookieManager
              .canSetCookie()
              .then(async (canSetCookie) => {
                try {
                  if (canSetCookie) {
                    await this.baseInterceptor.cookieManager.set(
                      ACCESS_TOKEN_COOKIE_NAME,
                      access_token,
                      {
