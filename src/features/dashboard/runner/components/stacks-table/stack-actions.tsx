...
type Props = {
  ...
  // whether this is asked for as one's own: a stack somebody else owns is not
...
export function StackActions({
  ...
  const [, deleteAction, isDeleting] = useActionState(
    own ? deleteMyStackAction : deleteStack,
    false,
  );
  ...
  const run = (command: StackCommand) => {
    ...
    startTransition(async () => {
      await (own ? commandMyStackAction : commandStack)(command, uuid);
    });
  };
