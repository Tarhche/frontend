...
export default class ExecutableCodeBlock extends Plugin {
  ...
  private _createDropdown() {
    ...
    clearModel.bind("isOn").to(command, "value", value => value === CLEAR_RUNTIME.value);
  ...
  execute( { value }: { value?: string | null } = {} ) {
    ...
    this.editor.model.change( writer => {
      if ( value ) {
        ...
        const htmlSupport = block.getAttribute( 'htmlContentAttributes' );
        if ( htmlSupport?.attributes?.[ `data-${ EXECUTABLE }` ] ) {
          ...
          Object.keys( attrs ).length
            ? writer.setAttribute( 'htmlContentAttributes', { attributes: attrs }, block )
